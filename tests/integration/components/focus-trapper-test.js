...
module('Integration | Component | focus-trapper', function(hooks) {
  ...
  test('it moves focus into the trapper', async function(assert) {
    ...
    assert.ok(this.element.contains(document.activeElement), 'when not disabled, focus is moved into the trapper');
  });
  ...
  test('disable works', async function (assert) {
    ...
    assert.ok(!this.element.contains(document.activeElement), 'when disabled, focus is not moved');
  });
  ...
  test('it traps focus', async function (assert) {
    ...
    assert.ok(this.element.contains(document.activeElement), 'it traps focus when tabbing out');

    await triggerKeyEvent('[name="trapped"]', 'keydown', '9', {shiftKey: true});
    assert.ok(this.element.contains(document.activeElement), 'it traps focus when shift+tabbing out');

    await click('[name="after"]');
    assert.ok(this.element.contains(document.activeElement), 'it traps focus when clicking out');
  });
});
